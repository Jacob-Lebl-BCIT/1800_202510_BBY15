...
// Initialize Firebase and load events when DOM content is loaded
...
async function saveEventsToFirestore() {
  ...
  try {
    ...
    // First, clear existing events in the subcollection
    ...
    eventsArr.forEach((dayEvent, index) => {
      const eventDoc = eventsRef.doc(`event_${index}`);
      batch.set(eventDoc, dayEvent);
    });
